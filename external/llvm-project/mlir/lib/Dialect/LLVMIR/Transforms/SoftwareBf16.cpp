...
struct UnaryBF16AsF32 : OpRewritePattern<Op> {
  explicit UnaryBF16AsF32(MLIRContext *context) : OpRewritePattern<Op>(context) {}
  ...
  LogicalResult matchAndRewrite(Op op,
    ...
    //for (Value v : op.getOperand()) {
      extended.push_back(rewriter.create<LLVM::FPExtOp>(loc, extType, op.getOperand()));
    //}
    Op operation = rewriter.create<Op>(loc, extType, extended, op->getAttrs());
    rewriter.replaceOpWithNewOp<LLVM::FPTruncOp>(op, resType,
                                                 operation.getResult());
...
struct SoftwareBF16Cmp : OpRewritePattern<LLVM::FCmpOp> {
  ...
  LogicalResult matchAndRewrite(LLVM::FCmpOp op,
    ...
    LLVM::FCmpOp operation = rewriter.create<LLVM::FCmpOp>(loc, resType, extended, op->getAttrs());
...
struct SoftwareBF16Trunc : OpRewritePattern<LLVM::FPTruncOp> {
  ...
  LogicalResult matchAndRewrite(LLVM::FPTruncOp op,
    ...
    Type bf16 = rewriter.getBF16Type();
    if (auto destShaped = destType.dyn_cast<ShapedType>()) {
      if (destShaped.getElementType() != i16 && destShaped.getElementType() != bf16)
        return failure();
...
struct ConvertBF16ToI16 : public ConvertOpToLLVMPattern<LLVMOp> {
  ...
  LogicalResult matchAndRewrite(LLVMOp op, typename LLVMOp::Adaptor adaptor,
                       ConversionPatternRewriter &rewriter) const override {
    assert(!adaptor.getOperands().empty());
    return oneToOneRewrite(op, LLVMOp::getOperationName(), adaptor.getOperands(),
...
void convertBF16Type(LLVMTypeConverter &converter,
  ...
  patterns.add<ConvertBF16ToI16<LLVM::FAddOp>>(converter);
}
...
void mlir::LLVM::populateBF16(LLVMTypeConverter &converter,
  ...
  converter.addConversion(
      [llvmI16](mlir::BFloat16Type type) -> Type { 
llvm::errs()<<"\ntype-convert a bf16\n"; 
return llvmI16; });
  ...
  patterns.add<LLVM::BF16ConstCasting, LLVM::SoftwareBF16Trunc, LLVM::SoftwareBF16Ext,
               LLVM::BF16AsF32<LLVM::FAddOp>, LLVM::BF16AsF32<LLVM::FMulOp>,
               LLVM::BF16AsF32<LLVM::FMAOp>>(ctx);

  patterns.add<LLVM::BF16AsF32<LLVM::FSubOp>, LLVM::BF16AsF32<LLVM::FDivOp>, LLVM::SoftwareBF16Cmp,
                   LLVM::UnaryBF16AsF32<LLVM::FAbsOp>>(ctx); 
}
...
struct SoftwareBF16Pass : public SoftwareBF16Base<SoftwareBF16Pass> {
  void runOnOperation() override {
     ...
     LLVMTypeConverter converter(ctx, nullptr);
